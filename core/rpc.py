...
Este modulo e o limite entre o terminal web e o Bitcoin Core. Ele interpreta a
linha digitada pelo operador, aplica allowlist/blocklist por rede, executa
chamadas HTTP com timeout/cache e resolve o comando local ``inspect_tx``.

Chamadas internas controladas, como a faucet Signet do backend, podem usar
``bypass_policy=True`` para operar uma wallet especifica sem expor esses
metodos ao terminal publico.
"""
...
READ_ONLY_METHODS = {
    "decodepsbt", "decoderawtransaction", "decodescript", "deriveaddresses",
    "estimatesmartfee", "getbestblockhash", "getblock", "getblockchaininfo",
    "getblockcount", "getblockfilter", "getblockhash", "getblockheader",
    "getblockstats", "getchaintips", "getchaintxstats", "getconnectioncount",
    "getdeploymentinfo", "getdifficulty", "getindexinfo", "getmempoolancestors",
    "getmempooldescendants", "getmempoolentry", "getmempoolinfo", "getmininginfo",
    "getnettotals", "getnetworkhashps", "getnetworkinfo", "getpeerinfo",
    "getrawmempool", "getrawtransaction", "getrpcinfo", "gettxout",
    "gettxoutproof", "gettxoutsetinfo", "help", "inspect_tx", "uptime",
    "validateaddress", "verifychain", "verifytxoutproof"
}
...
class RPCPolicyError(PermissionError):
    """Erro levantado quando uma rede/metodo viola a politica configurada."""
...
SIGNET_WALLET_METHODS = {
    "getnewaddress", "getbalance", "sendtoaddress",
    "listwallets", "loadwallet", "createwallet", "getwalletinfo"
}
SIGNET_ALLOWLIST = csv_set("SIGNET_RPC_ALLOWLIST", READ_ONLY_METHODS.union(SIGNET_WALLET_METHODS))
...
CACHEABLE_METHODS = {
    ...
    "getblockcount", "getmempoolinfo", "getnetworkinfo", "getpeerinfo",
}
...
def ensure_method_allowed(network, method):
    """Aplica allowlist/blocklist antes de chamadas expostas ao operador."""
...
def rpc_call(network, method, params=None, bypass_policy=False):
    """Executa um metodo JSON-RPC na rede selecionada.

    Por padrao, aplica a politica da rede antes de chamar o Bitcoin Core.
    ``bypass_policy`` existe para fluxos internos do backend, como a faucet
    Signet, que precisam carregar wallet e enviar valor fixo sem liberar esses
    metodos para o terminal do usuario.
    ...
    if normalized_method == "inspect_tx":
        ...
        if len(raw_data) == 64:
            ...
            try:
                resp = requests.post(
                    config["url"], auth=(config.get("user") or "", config.get("pass") or ""),
                    json=payload, timeout=RPC_TIMEOUT_SECONDS,
                )
